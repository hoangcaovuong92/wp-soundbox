...
if( empty( $payment ) ) : ?>
	...
	<div class="edd_errors edd-alert edd-alert-error">
		<?php _e( 'The specified receipt ID appears to be invalid', 'wdsoundbox' ); ?>
	</div>
...
<table id="edd_purchase_receipt" class="edd-table">
	<thead>
		...
		<tr>
			<th><strong><?php _e( 'Payment', 'wdsoundbox' ); ?>:</strong></th>
	...
	<tbody>
		...
		<tr>
			<td class="edd_receipt_payment_status"><strong><?php _e( 'Payment Status', 'wdsoundbox' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['payment_key'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Payment Key', 'wdsoundbox' ); ?>:</strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['payment_method'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			<tr>
				<td><strong><?php _e( 'Payment Method', 'wdsoundbox' ); ?>:</strong></td>
		...
		<tr>
			<td><strong><?php _e( 'Date', 'wdsoundbox' ); ?>:</strong></td>
		...
		<tr>
			<td><strong><?php _e( 'Fees', 'wdsoundbox' ); ?>:</strong></td>
			<td>
		...
		<?php if ( filter_var( $edd_receipt_args['discount'], FILTER_VALIDATE_BOOLEAN ) && isset( $user['discount'] ) && $user['discount'] != 'none' ) : ?>
			<tr>
				<td><strong><?php _e( 'Discount(s)', 'wdsoundbox' ); ?>:</strong></td>
		...
		<?php if( edd_use_taxes() ) : ?>
			<tr>
				<td><strong><?php _e( 'Tax', 'wdsoundbox' ); ?></strong></td>
		...
		<?php if ( filter_var( $edd_receipt_args['price'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
			...
			<tr>
				<td><strong><?php _e( 'Subtotal', 'wdsoundbox' ); ?></strong></td>
				<td>
			...
			<tr>
				<td><strong><?php _e( 'Total Price', 'wdsoundbox' ); ?>:</strong></td>
...
<?php if ( filter_var( $edd_receipt_args['products'], FILTER_VALIDATE_BOOLEAN ) ) : ?>
	...
	<h3><?php echo apply_filters( 'edd_payment_receipt_products_title', __( 'Products', 'wdsoundbox' ) ); ?></h3>

	<table id="edd_purchase_receipt_products" class="edd-table">
		<thead>
			<th><?php _e( 'Name', 'wdsoundbox' ); ?></th>
			<?php if ( edd_use_skus() ) { ?>
				<th><?php _e( 'SKU', 'wdsoundbox' ); ?></th>
			<?php } ?>
			<?php if ( edd_item_quantities_enabled() ) : ?>
				<th><?php _e( 'Quantity', 'wdsoundbox' ); ?></th>
			<?php endif; ?>
			<th><?php _e( 'Price', 'wdsoundbox' ); ?></th>
		</thead>
					...
					<td>
						...
						<?php if ( $edd_receipt_args['notes'] ) : ?>
							<div class="edd_purchase_receipt_product_notes">
								<?php echo edd_get_product_notes( $item['id'] ); ?>
						...
						<ul class="edd_purchase_receipt_files">
							...
							elseif( edd_is_bundled_product( $item['id'] ) ) :
									...
									<li class="edd_bundled_product">
										...
										<ul class="edd_bundled_product_files">
											...
											if( $download_files && is_array( $download_files ) ) :
												...
												echo '<li>' . __( 'No downloadable files found for this bundled item.', 'wdsoundbox' ) . '</li>';
											endif;
											?>
										</ul>
									</li>
								...
								echo '<li>' . apply_filters( 'edd_receipt_no_files_found_text', __( 'No downloadable files found.', 'wdsoundbox' ), $item['id'] ) . '</li>';
							endif; ?>
						</ul>
						<?php endif; ?>
