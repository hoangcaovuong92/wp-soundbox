<?php 
	class Tvlgiao_Wpdance_Admin_GeneralTheme extends Tvlgiao_Wpdance_GeneralTheme{
		...
		public function __construct(){
			$this->soundbax_constants();
			add_action('admin_init',array($this,'soundbax_load_javascript_style'));
			add_action('admin_enqueue_scripts',array($this,'soundbax_load_javascript_style'));
		...
		public function soundbax_constants(){
		...
		public function soundbax_load_javascript_style(){
			/*----------------- Style ---------------------*/
			wp_register_style( 	'fancybox-css', 			TVLGIAO_WPDANCE_THEME_INCLUDES_CSS .'/jquery.fancybox.css');
			wp_enqueue_style('fancybox-css');
			
			wp_register_style( 'tvlgiao-wpdance-style', 	TVLGIAO_WPDANCE_THEME_INCLUDES_CSS .'/wd_style.css');
			wp_enqueue_style('tvlgiao-wpdance-style');

			if (function_exists('wp_enqueue_media')) {
			wp_register_script('admin_media_lib_35', TVLGIAO_WPDANCE_THEME_INCLUDES_JS . '/admin-media-lib-35.js', 'jquery', false,false);
			wp_enqueue_script('admin_media_lib_35');
		} else {
			
			wp_register_script('admin_media_lib', TVLGIAO_WPDANCE_THEME_INCLUDES_JS . '/admin-media-lib.js', 'jquery', false,false);
