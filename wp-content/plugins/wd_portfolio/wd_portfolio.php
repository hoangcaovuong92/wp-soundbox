...
class WD_Portfolio {
	...
	public function wd_portfolio_item($atts = array()) {
		...
		public function wd_portfolio_custom_image($atts = array()) {
		...
		ob_start();
		show_item_portfolio_imag($id,$type,$word,$id_image,$height,$width,$slug);	
		$content = ob_get_clean();
		//ob_end_clean();
		return $content;
	}

	public function wd_portfolio_owlcarousel($atts = array()) {
		...
		ob_start();
		show_carousel_portfolio($limit);
		$content = ob_get_clean();
		//ob_end_clean();
		return $content;	
	}

	public function wd_portfolio_masonry($atts = array()) {

		extract(shortcode_atts(array(
			'limit' 				=> -1,
		),$atts));
		ob_start();		
		show_masonry_portfolio($limit);	
		$content = ob_get_clean();
		//ob_end_clean();
		return $content;
	}
	...
	protected function init_handle(){
		...
		add_shortcode('wd-portfolio-carousel', array( $this,'wd_portfolio_owlcarousel') );
		add_shortcode('wd-portfolio-item', array( $this,'wd_portfolio_item') );
		add_shortcode('wd-portfolio-masonry', array( $this,'wd_portfolio_masonry') );
		add_shortcode('wd-portfolio-image',array($this,'wd_portfolio_custom_image'));
	}	
