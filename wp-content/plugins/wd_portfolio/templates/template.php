						...
						<div id="portfolio-container" class="<?php echo "portfolio-".$portf_style."-style" ?>">
							<div id="portfolio-container-holder">	
								<div class="portfolio-galleries" id="portfolio-galleries">
									...
									<div id="portfolio-galleries-holder row">
									<?php	
										$title_icon = "";
										...
										$count=0;
										if($query->have_posts()) : while($query->have_posts()) : $query->the_post(); global $post;global $wp_query;
										...
										<div class=" <?php echo $class_span;?> item-portfolio<?php //if($count % $columns == 0)  echo " first"; ?><?php  //if($count % $columns == ($columns - 1) || ($count + 1) == $wp_query->post_count ){ echo " last";} ?>" data-type="<?php echo $term_list;?>" data-id="<?php echo $post->ID;?>">
											<div>	
...
function show_carousel_portfolio($limit = -1){
	$wd_portfolio = new WD_Portfolio();
	ob_start();
	$query = new WP_Query( array( 'post_type' => 'portfolio', 'posts_per_page' => $limit) );
	if($query->have_posts()) : ?>
	<div class="container-porfolio-slider">
		<?php $id = 'portfolio-slider-'.rand(10,100); ?>
		<div class="owl-carousel owl-theme" id= '<?php echo esc_attr($id); ?>'>
...
function show_item_portfolio_imag($id = '',$type_image='thumbnail',$number_word='60',$id_image='',$height='',$width='', $slug = ''){
	if( absint($id) > 0 ){
		$query = new WP_Query( array( 'post_type' => 'portfolio', 'post__in' => array($id )) );
	}elseif( strlen(trim($slug)) > 0 ){
		$_post = get_page_by_path($slug, OBJECT, 'portfolio');
		...
		<div class="portfolio_sc">
			...
			<div class="thumb-tag">
				<div class="wd_thumb-tag">
					<h2><a class="post-title heading-title list-title portfolio-grid-title" href="<?php echo $post_url; ?>">
						<?php echo $post_title; ?>
					</a></h2>
					<div class="wd_pl_des"><?php echo $post_description; ?></div>
					<div class="wd_pf_readmore"><a alt="<?php echo $post_title; ?>" title="<?php echo $post_title; ?>" href="<?php echo $post_url; ?>" ><?php _e('Learn more','wpdance'); ?></a></div>
