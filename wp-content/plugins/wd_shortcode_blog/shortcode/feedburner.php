...
if (!function_exists('wd_feedburner_shortcode')) {
	function wd_feedburner_shortcode($atts, $content = null) {
		...
		$html .='<span class="subscribe-email">
				<span class="title">'.$title.'</span></span>
				<div class="subscribe-form">
		<input type="text" name="email" class="subscribe_email" placeholder="'.__('enter your email address','wpdance').'" value=""/></span>
					<input type="hidden" value="'.$feedburner.'" name="uri"/>
					<input type="hidden" value="'.get_bloginfo( 'name' ).'" name="title"/>
					<input type="hidden" name="loc" value="en_US"/>';
		$html .= '<button class="btn" '.$button_out.' type="submit" title="submit"><i class="fa fa-paper-plane-o" aria-hidden="true"></i><span>'.$button_text.'</span></button></div>';
